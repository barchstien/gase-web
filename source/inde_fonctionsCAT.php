<?php
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT ID_CATEGORIE, NOM FROM _inde_CATEGORIES WHERE ID_CAT_SUP is NULL ORDER BY NOM");
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT ID_CATEGORIE, NOM FROM _inde_CATEGORIES ORDER BY NOM");
		while ( $row = $result->fetch_array())
		{
			$listeCategories[$row["ID_CATEGORIE"]] = $row["NOM"];
		}
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT NOM, ID_CAT_SUP, SOUS_CATEGORIES, VISIBLE FROM _inde_CATEGORIES WHERE ID_CATEGORIE = '$idCategorie'");
		while ( $row = $result->fetch_array())
		{		
			$donnees['NOM'] = $row[0];
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT c2.NOM FROM _inde_CATEGORIES c1, _inde_CATEGORIES c2 WHERE c1.ID_CATEGORIE = '$idCategorie' AND c2.ID_CATEGORIE = c1.ID_CAT_SUP");
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT ID_CAT_SUP FROM _inde_CATEGORIES WHERE ID_CATEGORIE = '$idCategorie'");
		while ( $row = $result->fetch_array())
		{		
			$id = $row['ID_CAT_SUP'];
		}
	...
	function MajSousCategorie($idCategorie, $nom, $visible, $NouvelleCatMere, $ancienneCatMere)
	{
		...
		$result = $connection->query("SELECT COUNT(*) FROM _inde_CATEGORIES c WHERE c.ID_CAT_SUP = '$ancienneCatMere'");
		while ( $row = $result->fetch_array())
		{		
			$nbre = $row[0];
		}
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT ID_CATEGORIE, NOM FROM _inde_CATEGORIES WHERE SOUS_CATEGORIES = 0 ORDER BY NOM");
		while ( $row = $result->fetch_array())
		{
			$listeCategories[$row["ID_CATEGORIE"]] = $row["NOM"];
		}
	...
	{
		$connection = ConnectionBDD();
		$result = $connection->query("SELECT NOM FROM _inde_CATEGORIES WHERE ID_CATEGORIE = '$idCategorie'");
		while ( $row = $result->fetch_array())
		{
			$nom = $row["NOM"];
		}
	...
	function SelectionListeCategoriesMenu()
	{
		...
		$compteur = 0;
		$visible = 1;
		$result = $connection->query("SELECT ID_CATEGORIE, NOM, ID_CAT_SUP, SOUS_CATEGORIES FROM _inde_CATEGORIES WHERE VISIBLE = '$visible' ORDER BY NOM");
		while ( $row = $result->fetch_array())
		{		
	...
	function SelectionListeSousCategories($idCategorie)
	{
		...
		$compteur = 0;
		$visible = 1;
		$result = $connection->query("SELECT ID_CATEGORIE, NOM FROM _inde_CATEGORIES WHERE ID_CAT_SUP = '$idCategorie' AND VISIBLE = '$visible' ORDER BY NOM");
		while ( $row = $result->fetch_array())
		{		
			$ligne['ID_CATEGORIE'] = $row[0];
			$ligne['NOM'] = $row[1];
